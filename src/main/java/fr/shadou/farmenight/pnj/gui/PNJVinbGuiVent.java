...
public class PNJVinbGuiVent extends Screen {
    ...
    private static ITextComponent textvent1 = new TranslationTextComponent("Raisin"+"        "+ Price.RaisinVent+"$");
    private static ITextComponent texteachat = new TranslationTextComponent("achat");

    public PNJVinbGuiVent(ITextComponent p_i51108_1_, PlayerEntity player) {
        super(p_i51108_1_);
        this.player = player;
    }
    ...
    protected void init() {
        ...
        //Vente
        addButton(new Button(relX + 13,relY + 33, 148,20,textvent1,button -> Minecraft.getInstance().setScreen(new ValidationGuiVent(NAME, Instas.ItemStackCustom(ModItem.RAISIN),0,0, Price.Raisin, this.player))));

        addButton(new Button(relX+125,relY+155,45,8,texteachat,button -> Minecraft.getInstance().setScreen(new PNJVinGui(NAME,player))));
    }
    ...
    public void newoffre(int index, int relX, int relY, RegistryObject<Item> item, int price, String nametexte){
        ...
        addButton(new Button(relX + 13,relY + 33+yadd, 148,20,textvent2,button -> Minecraft.getInstance().setScreen(new ValidationGuiVent(NAME, Instas.ItemStackCustom(item),0,0, price, this.player))));
    }
